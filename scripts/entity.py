...
class Enemy(PhysicsEntity):
    ...
    def update(self, tilemap, movement=(0, 0)):
        if self.walking:
            if tilemap.solid_check((self.rect().centerx + (-7 if self.flip else 7), self.pos[1] + 23)): # check if its theres a block is present
                if (self.collisions['left'] or self.collisions['right']): # turn if hit something
        ...
        if abs(self.game.player.dashing) >= 50:
            if self.rect().colliderect(self.game.player.rect()):
                ...
                self.game.sparks.append(Spark(self.rect(), 0, 5 + random.random()))
                self.game.sparks.append(Spark(self.rect(), math.pi, 5 + random.random()))
                return True
    
    def render(self, surf, offset=(0, 0)):
        super().render(surf, offset)

        if self.flip: # drawing the enemy gun [~4:28:00]
    ...
    def jump(self):
        if self.wall_slide:
            if self.flip and self.last_movement[0] < 0:
                ...
                return True    # for adding extra events if yo jump [3:46:40]
